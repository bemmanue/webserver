...
namespace ft {
  ...
  const char* validMainDirectives[] = {KW_SERVER, NULL};

  const char* validServerDirectives[] = {KW_CLIENT_MAX_BODY_SIZE,
                                         KW_ERROR_PAGE,
                                         KW_HOST,
                                         KW_LOCATION,
                                         KW_PORT,
  ...
  const char* validLocationDirectives[] = {
      KW_AUTOINDEX,       KW_CGI_PASS,      KW_INDEX,
      KW_METHODS_ALLOWED, KW_REDIRECT,      KW_ROOT,
      KW_OPENING_BRACE,   KW_CLOSING_BRACE, NULL};

  const char* validKeywords[] = {KW_AUTOINDEX,
                                 KW_CGI_PASS,
  ...
  std::string getNextToken() {
    ...
    skipSpace();
    for (; g_index < g_buffer.size(); ++g_index) {
      if (isspace(g_buffer[g_index]) || g_buffer[g_index] == ';' ||
          g_buffer[g_index] == '{' || g_buffer[g_index] == '}') {
        if (token.empty()) {
  ...
  std::string getLocationPath() {
    ...
    } else if (path == KW_SEMICOLON || path == KW_OPENING_BRACE ||
  ...
  void setAutoindex(LocationBlock & location,
  ...
  void setCGIs(LocationBlock & location,
               const std::vector<std::string>& params) {
    if (params.size() != 2) {
  ...
  void setIndex(LocationBlock & location,
                const std::vector<std::string>& params) {
  ...
  void setRedirect(LocationBlock & location,
                   const std::vector<std::string>& params) {
    if (params.size() != 2) {
  ...
  void setRoot(LocationBlock & location,
               const std::vector<std::string>& params) {
    if (params.size() != 1) {
  ...
  void setClientMaxBodySize(ServerBlock & server,
                            const std::vector<std::string>& params) {
    if (params.size() != 1) {
      throw InvalidNumberOfArgumentsConfigException(KW_CLIENT_MAX_BODY_SIZE,
                                                    g_line);
    }
  ...
  void setErrorPages(ServerBlock & server,
                     const std::vector<std::string>& params) {
    if (params.size() != 2) {
  ...
  Config parseConfigFile(const std::string& filename) {
    ...
    while (!(keyword = getNextToken()).empty()) {
      if (keyword == KW_OPENING_BRACE || keyword == KW_CLOSING_BRACE ||
